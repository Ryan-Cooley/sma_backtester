...
Usage:
    python run_backtest.py --ticker SPY --start 2015-01-01 --end 2024-01-01 --fast 20 --slow 50
    python run_backtest.py --ticker QQQ --fast 10 --slow 30 --cash 50000
"""
...
from metrics import (compute_cumulative_return, compute_sharpe_ratio, 
                    compute_max_drawdown, compute_rolling_metrics, plot_comprehensive_dashboard)

def run_backtest(ticker, start_date, end_date, fast_sma, slow_sma, 
                initial_cash, transaction_cost, verbose=True):
    """
...
def run_grid_search(ticker, start_date, end_date, fast_smas, slow_smas, 
    ...
    for fast in fast_smas:
        for slow in slow_smas:
            ...
            try:
                result = run_backtest(ticker, start_date, end_date, fast, slow,
                                    initial_cash, transaction_cost, verbose=False)
                results.append(result)
                print(f"SMA {fast}/{slow}: Return {result['cumulative_return']:.2%}, "
    ...
    for i, result in enumerate(results[:5]):
        print(f"{i+1}. SMA {result['fast_sma']}/{result['slow_sma']}: "
...
def main():
    ...
    parser.add_argument('--fast-range', nargs=2, type=int, default=[10, 30], 
                       help='Fast SMA range for grid search (min max)')
    parser.add_argument('--slow-range', nargs=2, type=int, default=[50, 200], 
    ...
    try:
        if args.grid:
            ...
            results = run_grid_search(args.ticker, args.start, args.end, 
                                    fast_smas, slow_smas, args.cash, args.cost)
            ...
            # Single backtest
            result = run_backtest(args.ticker, args.start, args.end, 
                                args.fast, args.slow, args.cash, args.cost)
            ...
            if args.plot:
                ...
                df = generate_signals(df)
                results = backtest_signals(df, initial_cash=args.cash, transaction_cost=args.cost)
                ...
                plot_comprehensive_dashboard(df, results['portfolio_value'], args.fast, args.slow, save_path=plot_filename)
